...
ToDoListForm = ScriptForm.extend({
	...
	//Павлюков - 2 функции для сохранения и вывода описаний по каждой задаче
	ДеревоЗадач_ПриАктивизацииСтроки: function (Элемент) {
		
		var ТекущаяСтрока = this.form.ЭлементыФормы.ДеревоЗадач.ТекущиеДанные;
        if ((ТекущаяСтрока) && (ТекущаяСтрока.Уровень() == 2))
        {
			this.form.ЭлементыФормы.Описание.Значение = ТекущаяСтрока.Описание;
		}
		
	},
	
	Описание_ПриИзменении: function(Элемент) {
		
		var ТекущаяСтрока = this.form.ЭлементыФормы.ДеревоЗадач.ТекущиеДанные;
        if ((ТекущаяСтрока) && (ТекущаяСтрока.Уровень() == 2))
        {
			ТекущаяСтрока.Описание = this.form.ЭлементыФормы.Описание.Значение;
		} 
